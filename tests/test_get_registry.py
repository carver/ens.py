...
def test_namehash_nameprep(ens, mocker, fake_hash):
    mocker.patch.object(ens.web3, 'sha3', side_effect=fake_hash)
...
def test_namehash_expand(ens, mocker, fake_hash):
    mocker.patch.object(ens.web3, 'sha3', side_effect=fake_hash)
...
def test_resolver(ens, mocker, hash1, addr1):
    ...
    assert ens.resolver('')._web3py_contract.address == addr1
...
def test_labelhash(ens):
    ...
    assert web3.toHex(labelhash) == '0x4f5b812789fc606be1b3b16908db13fc7a9adf7ca72641f84d75b47069d3d7f0'
